...
class ExplicitJoinTest extends BaseTester {
	...
	void explicitJoinTest() {
		...
		try {
			...
			List<Order> orders = entityManager.createQuery("SELECT o FROM Member m JOIN Order o on m.id = o.member.id", Order.class)
											  .getResultList();
//			select
...
//			ORDERS order1_
//			on (
//				member0_.MEMBER_ID=order1_.MEMBER_ID
//			   )
